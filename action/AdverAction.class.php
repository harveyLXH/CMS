...
";
        ...
        if($_object){
            foreach($_object as $_value){
                ...
                if(empty($_value->state)){
                    $_value->state = '<span class="red">[否]</span> | <a href="adver.php?action=state&type=ok&id='.$_value->id.'">确认</a>';
                }else{
    ...
    private function update(){
        if(isset($_POST['send'])){
            ...
            $this->_model->updateAdver()? Tool::alertLocation('恭喜，修改广告成功！',$_POST['prev_url']) : Tool::alertBack('很遗憾，修改失败！');
        ...
        if(isset($_GET['id'])){
            ...
            switch($_adver->type){
                ...
                case 2:
                    ...
                    $this->_tpl->assign('up',"<input type=\"button\" value=\"上传头部广告690x80\" onclick=\"centerWindow('../config/upfile.php?type=adver&size=690x80', 'upfile', '400', '120'); \">");
                    break;
                case 3:
                    $this->_tpl->assign('type3','checked=checked');
                    $this->_tpl->assign('pic','style="display:block"');
                    $this->_tpl->assign('up',"<input type=\"button\" value=\"上传侧栏广告270x200\" onclick=\"centerWindow('../config/upfile.php?type=adver&size=270x200', 'upfile', '400', '120'); \">");
                    break;
            }
