...
class VoteAction extends Action {
        ...
        if($_object){
            foreach($_object as $_value){
                if(empty($_value->state)){
                    ...
                    $_value->state = '<span class="green">[是]</span> | <a href="vote.php?action=state&type=cancel&id='.$_value->id.'">取消</a>';
                }
    ...
    private function update() {
        if(isset($_POST['send'])){
            $this->_model->id = $_GET['id'];
            $this->setAdd();
            $this->_model->updateVote()?Tool::alertLocation('恭喜，投票修改成功！',$_POST['prev_url']):Tool::alertBack('很遗憾，投票修改失败！');
        }
        if(isset($_GET['id'])){
            $this->_model->id = $_GET['id'];
            $_vote = $this->_model->getOneVote();
