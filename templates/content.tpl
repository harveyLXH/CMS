        ...
        {if $SearchContent}
            {foreach $SearchContent(key,value)}
                <tr>
                    ...
                    <td>{@value->count}</td>
                    <td>{@value->date}</td>
                    <td><a href="content.php?action=update&id={@value->id}">修改</a> | <a href="content.php?action=delete&id={@value->id}" onclick="return confirm('你真的要删除文档吗?')?true:false;">删除</a></td>
                </tr>
            {/foreach}
        {else}
	...
	<form name="content" method="post" action="?action=add">
		<table cellspacing="0"  class="content">
            ...
            <tr><td>栏　　目：<select name="nav">
                                <option value="" style="padding:0;">请选择一个栏目类别</option>{$nav}
			...
			<tr><td>标　　签：<input type="text" name="tag" class="text"/> ( * 每个标签用','隔开，总长度30位之内)</td></tr>
			<tr><td>关 键 字：<input type="text" name="keyword" class="text"/> ( * 每个关键字用','隔开，总长度30位之内)</td></tr>
			<tr><td>缩 略 图：<input type="text" name="thumbnail" class="text" readonly />
                            <input type="button" value="上传缩略图" onclick="centerWindow('../config/upfile.php?type=content','upfile','400','120');">
                            <img name="pic" style="display:none;"/> ( * 缩略图必须是jpg,gif,png,并且200K内)
                </td></tr>
			...
			<tr><td><span class="middle">内容摘要：</span>：<textarea name="info" class=""></textarea><span class="middle"> ( * 内容摘要200位之内)</span></td></tr>
            <tr class="ckeditor"><td><textarea id="TextArea1" name="content" class="ckeditor"></textarea></td></tr>
            <tr><td>评论选项：<input type="radio" name="commend" checked value="1"/>允许评论
...
{if $update}
    <form name="content" method="post" action="?action=update">
        ...
        <table cellspacing="0"  class="content">
            ...
            <tr><td>标　　签：<input type="text" name="tag" value="{$tag}" class="text"/> ( * 每个标签用','隔开，总长度30位之内)</td></tr>
            <tr><td>关 键 字：<input type="text" name="keyword" value="{$keyword}" class="text"/> ( * 每个关键字用','隔开，总长度30位之内)</td></tr>
            <tr><td>缩 略 图：<input type="text" name="thumbnail" value="{$thumbnail}" class="text" readonly />
                    <input type="button" value="上传缩略图" onclick="centerWindow('../templates/upfile.html','upfile','400','120');">
                    <img name="pic" src="{$thumbnail}" style="display:block;"/> ( * 缩略图必须是jpg,gif,png,并且200K内)
                </td></tr>
            <tr><td>文章来源：<input type="text" name="source" value="{$source}" class="text"/> ( * 文章来源20位之内)</td></tr>
            <tr><td>作　　者：<input type="text" name="author" value="{$author}" class="text"/> ( * 作者10位之内)</td></tr>
            <tr><td><span class="middle">内容摘要：</span>：<textarea name="info" class="">{$info}</textarea><span class="middle"> ( * 内容摘要200位之内)</span></td></tr>
