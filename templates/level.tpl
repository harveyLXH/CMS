...
<table cellspacing="0">
	...
	<tr>
		...
		<td><a href="level.php?action=update&id={@value->id}">修改</a> | <a href="level.php?action=delete&id={@value->id}" onclick="return confirm('你真的要删除等级吗?')?true:false;">删除</a></td>
	</tr>
	{/foreach}
	{else}
...
{if $update}
	<form method="post" name="add">
		...
		<table cellspacing="0" class="left">
			...
			<tr><td><input type="submit" name="send" value="修改等级" onclick="return checkorm();" class="submit level" /> [ <a href="{$prev_url}">返回列表</a> ]</td></tr>
