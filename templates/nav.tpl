...
<form method="post" action="nav.php?action=sort">
    <table cellspacing="0">
        ...
        <tr><td colspan="5">本类隶属：<strong>{$prev_name}</strong> [ <a href="nav.php?action=addchild&id={$id}">增加本类</a> ] [ <a href="{$prev_url}">返回主导航列表</a> ] </td></tr>
    </table>
 </form>
    <div id="page">{$page}</div>
{/if}
...
{if $add}
	<form method="post" name="add">
		...
		<table cellspacing="0" class="left">
			<tr><td>等级名称：<input type="text" name="nav_name" class="text" /> (*导航名称不得小于两位，不得大于20位！)</td></tr>
...
{if $update}
	<form method="post" name="add">
		...
		<table cellspacing="0" class="left">
			<tr><td>等级名称：<input type="text" name="nav_name" value="{$nav_name}" class="text" /> (*导航名称不得小于两位，不得大于20位！)</td></tr>
