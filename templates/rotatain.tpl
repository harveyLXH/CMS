...
<form method="post" name="content">
	<table cellspacing="0" class="left">
		<tr><td>轮 播 图：<input type="text" name="thumbnail" readonly class="text" />
                <input type="button" value="上传轮播图" onclick="centerWindow('../config/upfile.php?type=rotatain','upfile','400','120');">
                <img name="pic" style="display:none;"/> ( * 最佳大小是268X193或以上，轮播图必须是jpg,gif,png,并且200K内)
            </td></tr>
...
{if $update}
    <form method="post" name="content">
		...
		<table cellspacing="0" class="left">
			<tr><td>轮 播 图：<input type="text" value="{$thumbnail}" name="thumbnail" readonly class="text" />
					<input type="button" value="上传轮播图" onclick="centerWindow('../config/upfile.php?type=rotatain','upfile','400','120');">
					<img name="pic" src="{$thumbnail}" style="display:block;"/> ( * 最佳大小是268X193或以上，轮播图必须是jpg,gif,png,并且200K内)
				</td></tr>
            ...
            <tr><td><input type="submit" name="send" value="修改轮播器" onclick="return checkAddRotatain();" class="submit level" /> [ <a href="{$prev_url}">返回列表</a> ]</td></tr>
		</table>
    </form>
{/if}
