...
<table cellspacing="0">
	...
	<tr>
		...
		<td>{@value->email}</td>
		<td>{@value->state}</td>
		<td><a href="user.php?action=update&id={@value->id}">修改</a> | <a href="user.php?action=delete&id={@value->id}" onclick="return confirm('你真的要删除会员吗?')?true:false;">删除</a></td>
	</tr>
	{/foreach}
	{else}
...
{if $update}
	<form method="post" name="reg">
		...
		<table cellspacing="0" class="user">
			...
			<tr><td>电子邮件：<input type="text" class="text" name="email" value="{$email}"/> <span class="red">[必填]</span> ( *每个电子邮件只能注册一个ID )</td></tr>
			<tr><td>选择头像：<select name="face" onchange="sface();">
                                {$face}
					</select>
				</td></tr>
			...
			<tr><td><input type="submit" class="submit" name="send" onclick="return checkUpdate();" value="修改会员" /> [<a href="{$prev_url}">返回上一层</a>] </td></tr>
		</table>
	</form>
{/if}
