...
<table cellspacing="0">
	...
	<tr>
		...
		<td><a href="user.php?action=update&id=<?php echo $value->id?>">修改</a> | <a href="user.php?action=delete&id=<?php echo $value->id?>" onclick="return confirm('你真的要删除会员吗?')?true:false;">删除</a></td>
	</tr>
	<?php } ?>
	<?php } else { ?>
...
<?php if ($this->_vars['add']) {?>
	<form method="post" name="reg">
		<table cellspacing="0" class="user">
			...
			<tr><td>
					设置权限：<input type="radio" name="state" value="0"/> 被封杀的会员
							<input type="radio" name="state" value="1"/> 待审核的会员
							<input type="radio" name="state" value="2" checked /> 初始会员
...
<?php if ($this->_vars['update']) {?>
	<form method="post" name="reg">
		...
		<table cellspacing="0" class="user">
			<tr><td>用 户 名：<?php echo $this->_vars['user'];?></tr>
			<tr><td>密　　码：<input type="password" class="text" name="pass" /> ( *留空则不修改 )</td></tr>
			<tr><td>电子邮件：<input type="text" class="text" name="email" value="<?php echo $this->_vars['email'];?>"/> <span class="red">[必填]</span> ( *每个电子邮件只能注册一个ID )</td></tr>
			<tr><td>选择头像：<select name="face" onchange="sface();">
			...
			<tr><td>问题答案：<input type="text" class="text" name="answer" value="<?php echo $this->_vars['answer'];?>"/></td></tr>
			<tr><td>
					设置权限：<?php echo $this->_vars['state'];?>
				</td></tr>
			<tr><td><input type="submit" class="submit" name="send" onclick="return checkUpdate();" value="修改会员" /> [<a href="<?php echo $this->_vars['prev_url'];?>">返回上一层</a>] </td></tr>
		</table>
	</form>
<?php } ?>
